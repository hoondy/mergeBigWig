...
### modified from http://wresch.github.io/2014/01/31/merge-bigwig-files.html
### requires bigWigMerge and bedGraphToBigWig installed on PATH
### i.e.
### 1. download from http://hgdownload.soe.ucsc.edu/admin/exe/
### 2. add to PATH: export PATH="$PATH:/path/to/ucsctools"
...
trap "rm -f ${tmp}" EXIT
export LC_ALL=C
bigWigMerge "$@" stdout | awk -v f=$f 'BEGIN{OFS="\t"}{$4=f*$4; print}' | sort -S ${mem_mb}M -T $tmpdir -k1,1 -k2,2n > ${tmp} || fail "bigWigMerge failed"
